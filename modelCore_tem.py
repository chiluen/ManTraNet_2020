...
class Conv2DSymPadding(nn.Module) :
    ...
    def call( self, inputs ) :
        if ( isinstance( self.kernel_size, tuple ) ) :
            ...
            kh = kw = self.kernel_size
        ph, pw = kh//2, kw//2
        inputs_pad = F.pad( inputs, pad = [[0,0],[ph,ph],[pw,pw],[0,0]], mode='replicate')
        outputs = K.conv2d(
                inputs_pad,
                self.kernel,
...
class BayarConstraint( Constraint ) :
    ...
    def _initialize_mask( self, w ) :
        nb_rows, nb_cols, nb_inputs, nb_outputs = w.shape
        m = np.zeros([nb_rows, nb_cols, nb_inputs, nb_outputs]).astype('float32')
        m[nb_rows//2,nb_cols//2] = 1.
        self.mask = Variable( m, dtype='float32' )
        return
    def __call__( self, w ) :
        if self.mask is None :
            self._initialize_mask(w)
        w *= (1-self.mask)
        rest_sum = torch.sum(w, (0, 1))
        w /= rest_sum + torch.finfo(torch.float32).eps
        w -= self.mask
        return w
